...
function rejectRows(report: FinishedGoodsReport, activeSamples: number): string {
    ...
    return REJECT_CRITERIA.map((criterion) => {
        ...
        const sampleResults = criterion.sampleCount === 1
            ? `<td class="sample merged-sample" colspan="13">${isRejected(results[0]) ? 'X' : results[0] === true ? 'V' : ''}</td>`
            : Array.from({ length: 13 }, (_, index) => {
                if (index >= activeSamples || index >= criterion.sampleCount) return cell('', 'sample inactive-sample');
                return cell(isRejected(results[index]) ? 'X' : results[index] === true ? 'V' : '', 'sample');
            }).join('');
...
function analysisRows(report: FinishedGoodsReport): string {
    ...
    return ANALYSIS_PARAMETERS.map((parameter) => {
        ...
        if (nameParts) {
            ...
            const displayValue = allEmpty ? '-' : nameParts.map((_, i) => {
                const val = (values[i] ?? '').trim();
...
export function buildReportHtml(report: FinishedGoodsReport): string {
  ...
  <table class="info"><colgroup><col class="label"><col class="colon"><col class="value"><col class="right-label"><col class="right-colon"><col class="right-value"></colgroup><tr>${cell('Flavour', 'label')}${cell(':', 'colon')}${cell(report.flavour, 'value')}${cell('Total Lot', 'right-label')}${cell(':', 'right-colon')}${cell(`${report.totalLot || ''} / ${report.totalLotPcs || ''} pcs`, 'right-value')}</tr><tr>${cell('Negara', 'label')}${cell(':', 'colon')}${cell(report.country, 'value')}${cell('Sample Size', 'right-label')}${cell(':', 'right-colon')}${cell(`${activeSamples} / ${report.sampleSizePlan || ''}`, 'right-value')}</tr><tr>${cell('Distributor', 'label')}${cell(':', 'colon')}${cell(report.distributor, 'value')}${cell('AQL', 'right-label')}${cell(':', 'right-colon')}${cell(`${report.aqlPercentage || ''} %, A/R : ${report.aqlAcceptReject || ''}`, 'right-value')}</tr><tr>${cell('Kode Produksi', 'label')}${cell(':', 'colon')}${cell(`${report.productionCode || ''} ${report.productionCodeDetail || ''} ${report.productionCode3 || ''}`.trim(), 'value')}${cell('Pemeriksaan Halal', 'right-label')}${cell(':', 'right-colon')}${cell(`${report.halalPercentage || ''} %`, 'right-value')}</tr><tr>${cell('Kode Lokasi', 'label')}${cell(':', 'colon')}${cell(report.locationCode, 'value')}${cell('Tanggal Analisa', 'right-label')}${cell(':', 'right-colon')}${cell(report.analysisDate, 'right-value')}</tr></table>
  ...
  <table class="signatures"><tr><td colspan="2">Pemeriksa</td><td colspan="2">Mengetahui</td></tr><tr><td colspan="2" class="signature-name">${escapeHtml(report.inspectorName || '................................')}${report.oqcType === 'OQC Monitoring' && report.analystName ? ' / ' + escapeHtml(report.analystName) : ''}</td><td colspan="2" class="signature-name">${escapeHtml(report.approverName || '................................')}</td></tr><tr><td colspan="2" class="signature-line">____________________</td><td colspan="2" class="signature-line">____________________</td></tr><tr><td colspan="2" class="signature-role">QC FG Field${report.oqcType === 'OQC Monitoring' && report.analystName ? ' / Analis RMFG' : ''}</td><td colspan="2" class="signature-role">QC RM/FG Spv. / QC FG Sect. Spv.</td></tr></table>
